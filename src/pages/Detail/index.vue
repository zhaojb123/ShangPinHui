    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- 小图列表 -->
          <ImageList :skuImageList="skuInfo.skuImageList"></ImageList>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0"
                :class="{active:item01.isChecked==1}"
                v-for="item01 in item.spuSaleAttrValueList"
                :key="item01.id"
                @click="changeActive(item01,item.spuSaleAttrValueList)"
                >
                {{item01.saleAttrValueName}}
                </dd>
...
export default {
  ...
  methods: {
    changeActive (item01, spuSaleAttrValueList) {
    ...
    async addShopCart () {
      try {
        // 成功
        await this.$store.dispatch('addShopCart', { skuId: this.$route.params.id, skuNum: this.skuNum })
